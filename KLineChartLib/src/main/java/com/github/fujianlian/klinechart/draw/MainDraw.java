...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private Paint mSelectorBordePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public MainDraw(BaseKLineChartView view) {
        ...
        //设置矩形框边框的画笔属性
        mSelectorBordePaint.setStyle(Paint.Style.STROKE);//设置样式为空心矩形
        mSelectorBordePaint.setColor(view.getColor(R.color.chart_sel_x_line_color));
        mSelectorBordePaint.setStrokeWidth(2);
    }
...
//            }
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        float margin = ViewUtil.Dp2Px(mContext, 15);
        float width = 0;
        float left;
        ...
        strings.add("成交量:" + new BigValueFormatter().format(point.getVolume()));
        ...
        canvas.drawRoundRect(r, padding, padding, mSelectorBordePaint);
