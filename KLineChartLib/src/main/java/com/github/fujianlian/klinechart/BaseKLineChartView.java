...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private Paint mSelectBorderPointPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private void init() {
        ...
        //设置矩形框边框的画笔属性
        mSelectBorderPointPaint.setStyle(Paint.Style.STROKE);//设置样式为空心矩形
        mSelectBorderPointPaint.setColor(getColor(R.color.chart_sel_x_line_color));
        mSelectBorderPointPaint.setStrokeWidth(2);
    ...
    private void drawK(Canvas canvas) {
        ...
        if (isLongPress) {
            ...
            canvas.drawLine(x, mMainRect.top, x, mMainRect.bottom, mSelectedYLinePaint);//K线图竖线
            canvas.drawLine(-mTranslateX, y, -mTranslateX + mWidth / mScaleX, y, mSelectedXLinePaint);//K线图横线
            canvas.drawLine(x, mMainRect.bottom, x, mVolRect.bottom, mSelectedYLinePaint);//柱状图竖线
            if (mChildDraw != null) {
    ...
    private void drawText(Canvas canvas) {
        ...
        if (isLongPress) {
            ...
            if (translateXtoX(getX(mSelectedIndex)) < getChartWidth() / 2) {
                x = 0;
                canvas.drawRect(x, y - r, mTextPaint.measureText(text), y + r, mSelectPointPaint);//绘制矩形框背景
                canvas.drawRect(x, y - r, mTextPaint.measureText(text), y + r, mSelectBorderPointPaint);//绘制矩形框边框
            } else {
                x = mWidth - mTextPaint.measureText(text);
                canvas.drawRect(x, y - r, mWidth, y + r, mSelectPointPaint);
                canvas.drawRect(x, y - r, mWidth, y + r, mSelectBorderPointPaint);
            }
            canvas.drawText(text, x, fixTextY1(y), mTextPaint);

            //画x值
            String xtext = mAdapter.getDate(mSelectedIndex);
            ...
            canvas.drawRect(x - mTextPaint.measureText(xtext) / 2 - 5, y, x + mTextPaint.measureText(xtext) / 2 + 5, y + baseLine + r, mSelectPointPaint);
            canvas.drawRect(x - mTextPaint.measureText(xtext) / 2 - 5, y, x + mTextPaint.measureText(xtext) / 2 + 5, y + baseLine + r, mSelectBorderPointPaint);
            canvas.drawText(xtext, x - mTextPaint.measureText(xtext) / 2, y + baseLine + 5, mTextPaint);
        }
    }
